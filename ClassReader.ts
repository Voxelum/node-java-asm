    ...
    */
    ...
    public constructor(buffer: Buffer, off: number = 0, len: number = buffer.length) {
        ...
        for (let i: number = 1; i < n; ++i) {
            ...
            let size: number;
            switch ((buffer[index])) {
                case WriterConstant.FIELD:
                case WriterConstant.METH:
                case WriterConstant.IMETH:
                case WriterConstant.INT:
                case WriterConstant.FLOAT:
                case WriterConstant.NAME_TYPE:
                case WriterConstant.INDY:
                    size = 5;
                    break;
                case WriterConstant.LONG:
                case WriterConstant.DOUBLE:
                    size = 9;
                    ++i;
                    break;
    ...
    public accept(classVisitor: ClassVisitor, attrs: Attribute[] = [], flags: number = 0): void {
        ...
        let sourceDebug: string = null;
        let enclosingOwner: string = null;
        let enclosingName: string = null;
        let enclosingDesc: string = null;
        let anns: number = 0;
        let ianns: number = 0;
        let tanns: number = 0;
        let itanns: number = 0;
        ...
        for (let i: number = this.readUnsignedShort(u); i > 0; --i) {
            let attrName: string = this.readUTF8(u + 2, c);
            if (("SourceFile" === attrName)) {
                ...
                access |= Opcodes.ACC_SYNTHETIC | WriterConstant.ACC_SYNTHETIC_ATTRIBUTE;
        ...
        while ((attributes != null)) {
            let attr: Attribute = attributes.next;
    ...
    private readField(classVisitor: ClassVisitor, context: Context, u: number): number {
        ...
        let access: number = this.readUnsignedShort(u);
        let name: string = this.readUTF8(u + 2, c);
        let desc: string = this.readUTF8(u + 4, c);
        u += 6;
        let signature: string = null;
        let anns: number = 0;
        let ianns: number = 0;
        ...
        u += 2;
        let fv: FieldVisitor = classVisitor.visitField(access, name, desc, signature, value);
        if (fv == null) {
            return u;
        }
        if (ClassReader.ANNOTATIONS && anns !== 0) {
    ...
    private readMethod(classVisitor: ClassVisitor, context: Context, u: number): number {
        ...
        for (let i: number = this.readUnsignedShort(u); i > 0; --i) {
            ...
            if (("Code" === attrName)) {
                ...
                context.access |= Opcodes.ACC_SYNTHETIC | WriterConstant.ACC_SYNTHETIC_ATTRIBUTE;
            } else if (ClassReader.ANNOTATIONS && ("RuntimeInvisibleAnnotations" === attrName)) {
                ianns = u + 8;
            } else if (ClassReader.ANNOTATIONS && ("RuntimeInvisibleTypeAnnotations" === attrName)) {
                itanns = u + 8;
        ...
        u += 2;
        let mv: MethodVisitor = classVisitor.visitMethod(context.access, context.name, context.desc, signature, exceptions);
        if (mv == null) {
            return u;
        }
        if (ClassReader.WRITER && (mv != null && mv instanceof MethodWriter)) {
            let mw: MethodWriter = <MethodWriter>mv;
        ...
        if (ClassReader.ANNOTATIONS && dann !== 0) {
            let dv: AnnotationVisitor = mv.visitAnnotationDefault();
    ...
    private readCode(mv: MethodVisitor, context: Context, u: number) {
        ...
        while ((u < codeEnd)) {
            ...
            switch ((WriterConstant.TYPE[opcode])) {
                case WriterConstant.NOARG_INSN:
                case WriterConstant.IMPLVAR_INSN:
                    u += 1;
                    break;
                case WriterConstant.LABEL_INSN:
                    this.readLabel(offset + this.readShort(u + 1), labels);
                    u += 3;
                    break;
                case WriterConstant.ASM_LABEL_INSN:
                    this.readLabel(offset + this.readUnsignedShort(u + 1), labels);
                    u += 3;
                    break;
                case WriterConstant.LABELW_INSN:
                    this.readLabel(offset + this.readInt(u + 1), labels);
                    u += 5;
                    break;
                case WriterConstant.WIDE_INSN:
                    opcode = b[u + 1] & 255;
                ...
                case WriterConstant.LOOK_INSN:
                    ...
                    u += 8;
                    break;
                case WriterConstant.VAR_INSN:
                case WriterConstant.SBYTE_INSN:
                case WriterConstant.LDC_INSN:
                    u += 2;
                    break;
                case WriterConstant.SHORT_INSN:
                case WriterConstant.LDCW_INSN:
                case WriterConstant.FIELDORMETH_INSN:
                case WriterConstant.TYPE_INSN:
                case WriterConstant.IINC_INSN:
                    u += 3;
                    break;
                case WriterConstant.ITFMETH_INSN:
                case WriterConstant.INDYMETH_INSN:
                    u += 5;
                    break;
                default:
                    u += 4;
        ...
        for (let i: number = this.readUnsignedShort(u); i > 0; --i) {
            ...
            let type: string = this.readUTF8(this.items[this.readUnsignedShort(u + 8)], c);
            mv.visitTryCatchBlock(start, end, handler, type);
            u += 8;
        }
        u += 2;
        let tanns: number[] = null;
        let itanns: number[] = null;
        let tann: number = 0;
        let itann: number = 0;
        let ntoff: number = -1;
        let nitoff: number = -1;
                ...
                case WriterConstant.ASM_LABEL_INSN:
                    {
                        ...
                        if (opcode === Opcodes.GOTO || opcode === Opcodes.JSR) {
                            ...
                            if (ClassReader.FRAMES && stackMap !== 0 && (frame == null || frame.offset !== offset + 3)) {
                                mv.visitFrame(WriterConstant.F_INSERT, 0, null, 0, null);
                            }
                        }
                        u += 3;
                        break;
                ...
                case WriterConstant.TABL_INSN:
                    {
                        ...
                        (this['__jswref_0'] = mv).visitTableSwitchInsn.apply(this['__jswref_0'], [min, max, labels[label]].concat(<any[]>table));
                        break;
                    };
                case WriterConstant.LOOK_INSN:
                    {
                ...
                case WriterConstant.ITFMETH_INSN:
                    {
                        ...
                        let itf: boolean = b[cpIndex - 1] === WriterConstant.IMETH;
                        let iowner: string = this.readClass(cpIndex, c);
                        cpIndex = this.items[this.readUnsignedShort(cpIndex + 2)];
                        let iname: string = this.readUTF8(cpIndex, c);
                        let idesc: string = this.readUTF8(cpIndex + 2, c);
                ...
                case WriterConstant.INDYMETH_INSN:
                    {
                        ...
                        let iname: string = this.readUTF8(cpIndex, c);
                        let idesc: string = this.readUTF8(cpIndex + 2, c);
                        (this['__jswref_1'] = mv).visitInvokeDynamicInsn.apply(this['__jswref_1'], [iname, idesc, bsm].concat(<any[]>bsmArgs));
                        u += 5;
                        break;
                    };
                case WriterConstant.TYPE_INSN:
    ...
    private readTypeAnnotations(mv: MethodVisitor, context: Context, u: number, visible: boolean): number[] {
        ...
        for (let i: number = 0; i < offsets.length; ++i) {
            ...
            if ((target >>> 24) === 66) {
                let path: TypePath = pathLength === 0 ? null : new TypePath(this.buf, u);
                u += 1 + 2 * pathLength;
     ...
     */
    private readAnnotationValues(v: number, buf: number[], named: boolean, av: AnnotationVisitor): number {
     ...
     */
    private readAnnotationValue(v: number, buf: number[], name: string, av: AnnotationVisitor): number {
        let i: number;
        if (av == null) {
            ...
            case ('[').charCodeAt(0):
                ...
                switch ((this.buf[v++] & 255)) {
                    case ('B').charCodeAt(0):
                        ...
                        for (i = 0; i < size; i++) {
                            bv[i] = (<number>this.readInt(this.items[this.readUnsignedShort(v)]) | 0);
                            v += 3;
                        }
                        av.visit(name, bv);
                        --v;
                    ...
                    case ('S').charCodeAt(0):
                        ...
                        for (i = 0; i < size; i++) {
                            sv[i] = (<number>this.readInt(this.items[this.readUnsignedShort(v)]) | 0);
                            v += 3;
                        }
                        av.visit(name, sv);
                        --v;
     ...
     * attribute.
     */
    private readAttribute(attrs: Attribute[], type: string, off: number, len: number, buf: number[], codeOff: number, labels: Label[]): Attribute {
    ...
    public readConst(item: number, buf: number[]): any {
        ...
        switch ((this.buf[index - 1])) {
            ...
            default:
                ...
                let itf: boolean = this.buf[cpIndex - 1] === WriterConstant.IMETH;
